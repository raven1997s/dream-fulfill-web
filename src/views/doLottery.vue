	...
	<div class="lottery-container">
		...
		<div v-else class="flex-container">
			<el-card class="activity-card" :body-style="{ padding: '50px', width: '80%' }">
				<h2>{{ activeActivity.theme }}</h2>
				<p>活动开始时间: {{ activeActivity.startTime }}</p>
				<p>活动结束时间: {{ activeActivity.endTime }}</p>
				<p>活动节日: {{ activeActivity.holidayName }}</p>
				<p>活动创建人: {{ activeActivity.createUserName }}</p>
			</el-card>
...
const fetchUserPrizes = (activityId, userId) => {
